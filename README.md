...
After `library(admincleanr)`, exported functions are attached and can be called
directly. Use `admincleanr::fn()` only when you want a one-off call without
...
Exported functions compile to the same bytecode as equivalent code you paste into a `.R` file. After `library(admincleanr)`, you can call them directly (for example, `squish_character_columns()`). The package also supports explicit namespacing (`admincleanr::squish_character_columns()`), plus documentation, versioning, and **dependency declarations** (`Imports` / `Suggests`) so `install_github` resolves what you need. Runtime speed is essentially the same; the win is reproducibility and sharing without copying ad hoc scripts.
