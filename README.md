...
When installed, companion packages are auto-attached by `library(admincleanr)` so
you can call their exported functions directly (no `admincleanr_crunch::` /
`admincleanr_pipe::` prefix). Disable with:
