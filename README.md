...
- **Heuristic datetime coercion:** `coerce_best_datetime()`, `coerce_best_datetime_cols()` — best-effort parsing when formats are unknown.
- **Join-key exploration:** `pairwise_column_overlap()` — Jaccard overlap scoring to rank candidate merge columns.
