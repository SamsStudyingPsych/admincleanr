...
Exported functions compile to the same bytecode as equivalent code you paste into a `.R` file. After `library(admincleanr)`, exported functions are directly callable (for example `squish_character_columns()`), while `admincleanr::` remains available when you want explicit namespacing. Runtime speed is essentially the same; the win is reproducibility and sharing without copying ad hoc scripts.
