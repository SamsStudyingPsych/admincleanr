...
Attach once per session, then call exported helpers directly without `admincleanr::`:
...
Exported functions compile to the same bytecode as equivalent code you paste into a `.R` file. After `library(admincleanr)`, those exported helpers are available directly (for example, `clean_but_keep()`); use `admincleanr::` only for one-off calls when you do not want to attach the package. The package still adds documentation, versioning, and **dependency declarations** (`Imports` / `Suggests`) so `install_github` resolves what you need. Runtime speed is essentially the same; the win is reproducibility and sharing without copying ad hoc scripts.
