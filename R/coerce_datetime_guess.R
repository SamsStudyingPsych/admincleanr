...
#' Ad hoc exploration and one-off extracts.
#' Once formats are understood, encode them explicitly (see
#' [lubridate::parse_date_time()] / SQL casting in production pipelines).
#'
#' @section Limitations:
#' \itemize{
...
#' \itemize{
#'   \item **Exploratory** pass on new string-date columns before you lock an explicit \code{parse_date_time} order in a production script.
#'   \item See \code{\link{admincleanr_training}} for extract-to-report sequencing with the main package.
#' }
#'
...
#'   \item See \code{\link{admincleanr_training}} for when to stop using heuristics and fix formats explicitly.
#' }
#'
