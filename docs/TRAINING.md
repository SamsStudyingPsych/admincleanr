...
This page is meant to be read in a browser **or** from your IDE after `library(admincleanr)` via `admincleanr_training("local")` (uses the copy shipped in the installed package). It stays **generic**—no employer field names or schema detail.
...
The main package exports the day-to-day cleaning helpers and exploratory helpers, so after `library(admincleanr)` you can call them directly without `admincleanr::`.

Optional **fast / heuristic** tools (datetime guessing, join-overlap exploration) are also kept in a sibling package built from the same repository:
