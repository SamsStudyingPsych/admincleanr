...
| ----- | ---- | ----------- |
| **Exploratory / heuristic** | Guess datetime formats, score candidate join keys, fuzzy matching | Ad hoc investigation, new feeds, unknown column semantics. |
| **Production / explicit** | Named formats, stable keys, validated transforms | Refreshable jobs, anything you must trust after a DBA change. |

Heuristic helpers (`coerce_best_datetime`, `pairwise_column_overlap`) trade **CPU and ambiguity** for speed of iteration. Production code should name formats, keys, and transforms explicitly once you know them.
...
The question is only: *after you block, roughly how many rows or pairs are left?* That number guides settings like `max_dist` and whether a step is exploratory or controlled production.
...
- Add production-hardened readers and validators as APIs stabilize.
