...
This page is meant to be read in a browser **or** from your IDE via `admincleanr_training("local")` after `library(admincleanr)` (uses the copy shipped in the installed package). Without attaching the package, use `admincleanr::admincleanr_training("local")` instead. It stays **generic**—no employer field names or schema detail.
